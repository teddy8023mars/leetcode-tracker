    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-baseline gap-3 flex-wrap">
          <h1 className="text-3xl font-extrabold tracking-tight">{c.nameEn}</h1>
          {c.region && (
            <span className="text-xs font-mono text-ink-soft uppercase">{c.region}</span>
          )}
          <span className="text-sm text-ink-soft font-mono">
            {t('list.problemCount', { count: c.problemCount ?? probs.length })}
          </span>
        </div>
      </div>
